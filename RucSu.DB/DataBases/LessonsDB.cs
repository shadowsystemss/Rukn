...
public class LessonsDB(DBContext db)
{
...
WHERE value = '{group}' AND date BETWEEN '{start:yyyy-MM-dd}' AND '{end:yyyy-MM-dd}'");
    ...
    {
        using SqliteCommand lessonAddCommand = db._connection.CreateCommand();
        lessonAddCommand.CommandText =
...
SELECT last_insert_rowid()";
        ...
        using SqliteCommand positionAddCommand = db._connection.CreateCommand();
        positionAddCommand.CommandText =
...
VALUES(@lessonId,@room,@type)";
        ...
        using SqliteCommand groupAddCommand = db._connection.CreateCommand();
        groupAddCommand.CommandText =
...
VALUES(@lessonId,@value)";
    ...
    public IList<ILesson>? ReadLessons(SqliteDataReader lessonReader)
    {
        ...
        SqliteCommand getPositionsCommand = db._connection.CreateCommand();
...
WHERE lessonId = @lessonId";
        ...
        SqliteCommand getGroupsCommand = db._connection.CreateCommand();
        getPositionsCommand.CommandText =
$@"SELECT value
FROM groups
WHERE lessonId = @lessonId";
